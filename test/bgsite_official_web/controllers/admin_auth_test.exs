defmodule BgsiteOfficialWeb.AdminAuthTest do
  use BgsiteOfficialWeb.ConnCase, async: true

  alias BgsiteOfficial.Accounts
  alias BgsiteOfficialWeb.AdminAuth
  import BgsiteOfficial.AccountsFixtures

  @remember_me_cookie "_bgsite_official_web_admin_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, BgsiteOfficialWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_admin/3" do
    test "stores the admin token in the session", %{conn: conn, admin: admin} do
      ...
      assert get_session(conn, :live_socket_id) == "admin_sessions:#{Base.url_encode64(token)}"
  ...
  describe "logout_admin/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "admin_sessions:abcdef-token"
      BgsiteOfficialWeb.Endpoint.subscribe(live_socket_id)
      ...
      assert_receive %Phoenix.Socket.Broadcast{
        event: "disconnect",
        topic: "admin_sessions:abcdef-token"
      }
    end
