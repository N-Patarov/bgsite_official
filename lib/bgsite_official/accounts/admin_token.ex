defmodule BgsiteOfficial.Accounts.AdminToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  schema "admin_tokens" do
    ...
    belongs_to :admin, BgsiteOfficial.Accounts.Admin
  ...
  def build_session_token(admin) do
    ...
    {token, %BgsiteOfficial.Accounts.AdminToken{token: token, context: "session", admin_id: admin.id}}
  end
  ...
  defp build_hashed_token(admin, context, sent_to) do
    ...
    {Base.url_encode64(token, padding: false),
     %BgsiteOfficial.Accounts.AdminToken{
       token: hashed_token,
       context: context,
       sent_to: sent_to,
       admin_id: admin.id
  ...
  def token_and_context_query(token, context) do
    from BgsiteOfficial.Accounts.AdminToken, where: [token: ^token, context: ^context]
  end
  ...
  def admin_and_contexts_query(admin, :all) do
    from t in BgsiteOfficial.Accounts.AdminToken, where: t.admin_id == ^admin.id
  end

  def admin_and_contexts_query(admin, [_ | _] = contexts) do
    from t in BgsiteOfficial.Accounts.AdminToken, where: t.admin_id == ^admin.id and t.context in ^contexts
  end
end
